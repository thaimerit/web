...
export default function Transaction({ session }) {
    ...
    async function initData() {
        ...
        try {
            ...
            if(order.partnerAcceptStatus=="accepted"){
                router.replace(`/liff/orders/${id}`);
                return
            }
    ...
    function onSubmit() {
        ...
        MySwal.fire({
            ...
            if (result.isConfirmed) {
                ...
                try {
                    await API.partnerOrder_Accepted(order.id)
                    ...
                    MySwal.fire({
                        ...
                        router.replace("/liff/pending-orders")
                    })
    ...
    function goBack() {
        router.push("/liff/pending-orders");
    }
    ...
    return (
        <LocalizationProvider dateAdapter={AdapterMoment}>
            <div>
                {/* <div className="bg-white drop-shadow-lg h-[4.2rem] grid grid-cols-8 border-1 grid content-center">
                    <div className="col-span-1 text-center">
                        <ChevronLeftOutlinedIcon onClick={goBack} />
                    </div>
                    <div className="col-span-6"><p className="text-center">รายละเอียด</p></div>
                </div> */}
                ...
                <div className="p-5">
                    ...
                    {order.status == "cancel" && <div className="mb-8 text-center">
                        <CheckCircleOutlineOutlinedIcon sx={[{ fontSize: 80 }, { color: '#F00' }]} />
                        <div style={{ fontSize: 30, color: '#F00' }}>ยกเลิกแล้ว</div>
            ...
            {order.status == "approve" && <div style={{alignItem: "center",position: "absolute",bottom:0,width:"100%",padding:20}}><Button style={{position:"absolute",marginBottom:0}} type="button" className="w-full mt-4 px-5 rounded linear-bg text-xl" loading={loading} onClick={onSubmit}>
                        รับงาน
                    </Button></div>}
