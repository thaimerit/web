...
function EventCard({ data, index, loading }) {
    return <a href={data.url} className={'inline-block relative ' + styles.eventCardNew}>
        <div className={"w-full rounded-xl " + styles.eventImageNew + (loading ? " animate-pulse " : "")}>
            <img src={data.thumbnail} className={"w-full rounded-xl " + styles.eventImageNew + (loading ? " animate-pulse " : "")} />
            {data.status == "complete" && <div className={styles.eventCardNewComplete}>
                <div className={styles.eventCardNewCompleteWrapper}>
                    <CheckCircleOutlineOutlinedIcon sx={[{ fontSize: 80 }, { color: '#61CC7F' }]} />
                    <div style={{ fontSize: 18, color: '#61CC7F' }}>จบงานแล้ว</div>
                </div>
            </div>}
        </div>
        <div className={styles.cardDetail}>
            <div className={styles.cardSubTitle}>{data.subTitle}</div>
            <h4 className={styles.cardTitleNew} dangerouslySetInnerHTML={{ __html: filterHtmlUtil(data.title) }}></h4>
...
function BlankCard({ text }) {
    ...
    return <div className={'rounded-xl inline-block relative bg-gray-200 text-center ' + styles.eventCardNew + " " + styles.blankCardNew}>
        <div className={styles.blankCardText}>{text}</div>
    </div>
}

export default function MiniSlideNew({ className, title, data, emptyText, customStyles, customHeader, loading }) {
    ...
    return <div className={className}>
        ...
        >
            {data.map((item, key) => <SwiperSlide key={key} className={'inline-block relative ' + styles.eventCardSwiper}>
                <EventCard data={item} key={key} index={key} loading={loading} />
            </SwiperSlide>)}

            {(loading == false && emptyText && data.length == 0) && <SwiperSlide className={'inline-block relative ' + styles.eventCardSwiper}>
